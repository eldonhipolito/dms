...
public class DocumentAccessServiceImpl implements DocumentAccessService {
	...
	public DocumentAccessServiceImpl(DocumentRepository documentRepository, UserRepository userRepository,
			DocumentAccessRepository documentAccessRepository, Random randomizer,
	...
	public String grantAccess(CreateDocumentAccessRequest request) {
		...
		Optional<DocumentAccess> docAccess = this.documentAccessRepository
				.findByUserIdAndDocumentId(owner.getId().intValue(), request.getDocumentId());
		...
		try {
			...
			byte[] masterKey = this.encryptionService.decrypt(
			...
			String tempEncryptedKey = new String(Base64.getEncoder().encode(this.encryptionService.encrypt(masterKey,
					Hex.decode(this.sha256HashingStrategy.hash(randomCode)))));

			DocumentAccess newDocAccess = new DocumentAccess(0L, viewer, doc.get(), tempEncryptedKey, 1,
